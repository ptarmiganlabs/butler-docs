...
::: info Lineage data
The response payload from both endpoints also includes a root-level `appId` and, when the connected Qlik Sense Engine supports it, an optional top-level `lineage` object populated from the Engine's `GetLineage` call. See [App dump lineage data](/v17.0/reference/rest-api/app-dump-lineage) for the response shape and the meaning of the `qDiscriminator` and `qStatement` fields.
:::
