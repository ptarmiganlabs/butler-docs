...
More info in the [Concepts section](/v17.0/concepts/reload-tasks/).
...
More info in the [Concepts section](/v17.0/concepts/successful-reloads).
...
Starting reload tasks using REST API is described [here](/v17.0/examples/start-task/start-task-from-rest).  
Using MQTT messages to achieve this is described [here](/v17.0/examples/start-task/start-task-from-mqtt).

## Passing parameters between reload tasks

This has always been hard both in QlikView and Sense.

Butler's [key-value store](/v17.0/concepts/key-value) makes it much easier to pass values from one app to the next in a reload chain.
...
The scheduler can be controlled via a separate config file on disk or Butler's [REST API](/v17.0/concepts/scheduler/).
...
More info [here](/v17.0/examples/file-operations).
...
The [REST API documentation](/v17.0/reference/rest-api) has full docs on this.
...
More info [here](/v17.0/reference/rest-api).
...
More info [here](/v17.0/concepts/qlik-sense-licenses/access-licenses).
