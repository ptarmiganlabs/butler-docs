...
When Butler is configured with `Butler.restServerConfig.tls.enable: true`, both the API base URL and the `/documentation` URL automatically switch from `http://` to `https://` (e.g. `https://192.168.1.168:8443/documentation`). See [Enabling HTTPS/TLS for the REST API](/v17.0/getting-started/setup/rest-api/https-tls) for details.
:::
...
Let's take a look at which API endpoints are [enabled in the restServerEndpointsEnable section of the config file](/v17.0/reference/config-file):
