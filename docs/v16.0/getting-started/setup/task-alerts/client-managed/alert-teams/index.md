...
See the [Concepts section](/v16.0/concepts/teams-messaging) for additional details.

A complete reference to the config file format is found [here](/v16.0/reference/config-file).
...
1. Create a workflow in Teams, take note of its URL (you will need it in step 2 below). More information on how to create a Teams workflow in the [Concepts section](/v16.0/concepts/teams-messaging).
...
3. Edit the template file if/as needed, the file is specified in `Butler.teamsNotification.reloadTaskFailure.templateFile`.It uses the Handlebars templating engine, to which Butler provides template fields with actual values.
   ...
   The available template fields are described [here](/v16.0/reference/alert-template-fields/).
...
2. Edit the Teams section of the config file i.e. the settings in `Butler.teamsNotification.reloadTaskFailure` and/or `Butler.teamsNotification.reloadTaskAborted` sections of the config file.
   ...
   The `basicMsgTemplate` property is the message that will be sent via Teams. [Template fields](/v16.0/reference/alert-template-fields/) can be used.
...
The concept is the same as for [all alert types](/v16.0/getting-started/setup/task-alerts/#how-it-works).
